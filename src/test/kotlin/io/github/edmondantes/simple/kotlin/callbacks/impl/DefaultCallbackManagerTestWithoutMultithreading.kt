...
class DefaultCallbackManagerTestWithoutMultithreading {

    private val callbackManager = DefaultCallbackManager<Any>(scheduler, null)
    ...
    fun simpleInvokeTest() {
        ...
        val callback = object : Callback<Any> {
            override fun execute(context: CallbackContext<Any>) {
                assertEquals(randomInt, context.data, "Income data and expected was different")
            }

            override fun cancel() {
                fail<Any>("Callback was cancelled")
            }
        }

        callbackManager.add(callback, 0)
        Thread.sleep(1000)
        callbackManager.invoke(randomInt)
    }
    ...
    fun testPriority() {
        ...
        val callback1 = object : Callback<Any> {
            ...
            override fun cancel() {
                fail<Any>("Callback with priority 1 was cancelled")
            }
        }
        ...
        val callback2 = object : Callback<Any> {
            ...
            override fun cancel() {
                fail<Any>("Callback with priority 2 was cancelled")
            }
        }
    ...
    fun testCallbackIsolations() {
        ...
        assertTrue(secondCallbackIsExecuted, "Second callback wasn't called")
    }

    @Test
    fun testCancellingByTimeout() {
        val callback = object : Callback<Any> {
            override fun execute(context: CallbackContext<Any>) {
                fail<Any>("Callback was called")
            }

            override fun cancel() {}
        }

        callbackManager.add(callback, 0, 1000)
        Thread.sleep(2000)
        callbackManager.invoke(0)
    }
